...
//
// Gioca UNA partita del motore contro se stesso (entrambi i colori usano
...
//
// Uso:
//   SelfPlay <model.pt> <output.jsonl> [iterations] [maxPlies] [seed] [openingPlies] [game_id]
//   default: 400 iterazioni/mossa, 200 ply massimi, seed casuale, 6 ply di
//   apertura casuale, game_id 0
...
int main(int argc, char** argv)
{
    if (argc < 3)
    {
        std::cerr << "Uso: SelfPlay <model.pt> <output.jsonl> [iterations] [maxPlies] [seed] [openingPlies] [game_id]\n";
        return 1;
    }
    ...
    std::cerr << "Partita " << gameId << " terminata: " << resultStr
              ...
              << records.size() << " posizioni scritte su " << outputPath << ")\n";
